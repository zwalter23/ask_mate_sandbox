...
@app.route("/")
def list():
        question = data_handler.reader("sample_data/question.csv")
        quest = []
...
def visitor(question_id):
    ...
    for row in question:
        if question_id == row["id"]:
            ...
            data_handler.edit(row, "sample_data/question.csv")
...
def quest(question_id):
    question = data_handler.reader("sample_data/question.csv")
    answer = data_handler.reader("sample_data/answer.csv")
    return render_template("question.html", question = question, answer = answer, id = question_id)
...
def new_answer(question_id):
    questions = data_handler.reader("sample_data/question.csv")
...
def save_answer(question_id):
    if request.method == "POST":
        id = data_handler.id_generator("sample_data/answer.csv")
        ...
        data_handler.writer(new_answer, "sample_data/answer.csv", "a")
    return redirect(f"/question/{question_id}")
...
def save():
    if request.method == "POST":
        id = data_handler.id_generator("sample_data/question.csv")
        ...
        data_handler.writer(question, "sample_data/question.csv", "a")
...
def edit_quest(id):
    ...
    data_handler.edit(question, "sample_data/question.csv")
    return redirect(f"/question/{id}")
...
def answer_vote_down(answer_id, question_id):
    ...
    for row in answer:
        if row["id"] == answer_id:
            if int(row["vote_number"]) > 0:
                row["vote_number"] = int(row["vote_number"]) - 1
                data_handler.edit(row, "sample_data/answer.csv")
    return redirect(f"/question/{question_id}")
